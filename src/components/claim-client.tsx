...
export function ClaimClient({ packetId }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/packets/${packetId}`)
      ...
      .then((data: Packet) => {
        setPacket(data);
        setStatus(data.funded ? "Ready to claim" : "Awaiting funding");
      })
      .catch(() => setStatus("Could not load packet"));
  }, [packetId]);
          ...
          <div className="rounded-[1.75rem] border border-stone-200 bg-white p-6 shadow-sm">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="rounded-2xl bg-stone-50 p-4">
                ...
                <input value={claimerEns} onChange={(event) => setClaimerEns(event.target.value)} className="mt-1 w-full bg-transparent text-lg font-semibold text-stone-950 outline-none" />
              </label>
              <label className="rounded-2xl bg-stone-50 p-4">
                <span className="text-sm text-stone-500">World ID nullifier</span>
                <input value={worldIdNullifier} onChange={(event) => setWorldIdNullifier(event.target.value)} className="mt-1 w-full bg-transparent text-lg font-semibold text-stone-950 outline-none" />
              </label>
            </div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <button onClick={handleClaim} disabled={isSubmitting || !packet?.funded} className="rounded-full bg-gradient-to-r from-red-600 to-amber-500 px-6 py-4 text-sm font-semibold text-white shadow-lg shadow-red-200 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60">
                {isSubmitting ? "Claiming..." : packet?.funded ? "Claim with World ID" : "Awaiting funding"}
              </button>
